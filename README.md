...
- Optional PostgreSQL backend for the auth, queue, and per-account databases
  (bstore remains the default; PG is opt-in via `mox.conf`).
...
By default mox stores all metadata in embedded
[bstore](https://github.com/mjl-/bstore) databases (one per account, one for
auth, one for the queue). This requires no external infrastructure and is the
recommended setup for most installations.

For deployments that need external database management — centrally-managed
backups with `pg_dump`, read replicas, or easier integration with existing DBA
tooling — mox also supports PostgreSQL (version 14 or later) as an optional
backend. Message files always remain on the local filesystem regardless of
which database backend is used.

## What moves to PostgreSQL

| Data | Default (bstore) | With PostgreSQL |
|------|-----------------|----------------|
| Per-account metadata (messages, mailboxes, flags, …) | `data/accounts/<name>/index.db` | schema `account_<name>` |
| Auth data (TLS keys, login attempts, sessions) | `data/auth.db` | schema `auth` |
| Queue data (outgoing messages, webhooks, suppressions) | `data/queue/index.db` | schema `queue` |
| Message files | `data/accounts/<name>/msg/…` | unchanged — still on disk |
| DMARC, MTA-STS, TLS-RPT databases | `data/*.db` | unchanged — still bstore |

## Setup

### 1. Create the PostgreSQL database

Option A — Docker (recommended for local/dev use):
    ...
    docker run -d \
      ...
      -e POSTGRES_DB=mox \
      -p 5432:5432 \
      -v /var/lib/mox-pgdata:/var/lib/postgresql/data \
      postgres:16

Option B — existing PostgreSQL instance:

    psql -U postgres <<'EOF'
    CREATE USER mox WITH PASSWORD 'secret';
    CREATE DATABASE mox OWNER mox;
    EOF

### 2. Configure mox

Add a `PostgreSQL` block to `mox.conf`:

    PostgreSQL:
        # Connection string in libpq URI or keyword=value form.
        DSN: postgres://mox:secret@localhost:5432/mox?sslmode=require
        # Or keep credentials in a separate file (path relative to mox.conf):
        # DSNFile: postgresql-dsn.txt
        MaxConns: 10    # optional, default 10
        MinConns: 0     # optional, default 0

Mutually exclusive options `DSN` and `DSNFile` are both optional in the config
struct but exactly one must be provided. `DSNFile` is useful to keep credentials
out of the main config file; the file should contain just the connection string,
whitespace-trimmed.

### 3. Migrate existing data (first time only)

If you have an existing bstore installation, migrate its data to PostgreSQL
while mox is stopped:

    # Stop mox first, then:
    mox pg migrate

This reads every record from the bstore files and writes them into PostgreSQL.
Each type is migrated in a single transaction; a failure rolls back that type
and reports the error while other types continue.

To preview what would be migrated without writing anything:

    mox pg migrate --dry-run

After a successful migration, restart mox. Verify normal operation before
removing the old `.db` files.

### 4. Start mox

On startup mox automatically runs any missing schema migrations
(`EnsureSchema`) for each component (`auth`, `queue`, `account_<name>`) before
opening the database. This is idempotent — already-applied versions are skipped
— so a restart after a partial failure or an upgrade is safe.

## Backup

With the bstore backend, `mox backup <destdir>` copies the `.db` files as part
of the backup. With PostgreSQL, **database content is not copied by `mox
backup`** — the command prints a note and skips the DB file-copy steps. Back up
PostgreSQL data with `pg_dump`:

    pg_dump -Fc -U mox mox > mox-$(date +%Y%m%d).pgdump

Message files (`data/accounts/*/msg/`) must still be backed up separately with
`mox backup` or any file-level backup tool; they are not stored in PostgreSQL.

## Verifying a backup

`mox verifydata <datadir>` detects PostgreSQL mode when `queue/index.db` is
absent from the data directory and skips the bstore/BoltDB integrity checks. It
still walks all message-file directories to check for unrecognized files. To
verify the database content itself, use `pg_restore --list` or restore to a
test instance and run queries.

## Local development with PostgreSQL

Use the `-pg` flag to point `mox localserve` at a PostgreSQL instance without
editing any config file:

    mox localserve -pg "postgres://user:pass@localhost:5432/moxtest?sslmode=disable"

Schemas are created automatically on first start. The flag overrides any
`PostgreSQL:` block already present in the localserve config file.
